...
bid,bidVol,ask,askVol = get_current_vol()
#%%
import matplotlib.pyplot as plt
...
def current_vol_plot(bid,bidVol,ask,askVol,alpha=1e-04):
    ...
    plt.show()
current_vol_plot(bid,bidVol,ask,askVol,alpha=1e-04)
#%%
...
#%%
def current_trd_plot(lastPrice, trd_vol):
    df = pd.Series(trd_vol)
    df.sort_index(inplace=True)
    plt.plot(df.index,df.values)
    plt.show()
current_trd_plot(lastPrice, trd_vol)
#%%
def trd_count(alpha=1e-4,tick=True):
    OrderQue_test = OrderQue(str(ins))
    ask_order = {}
    bid_order = {}
    for data_i in getMkDataMult(dataStreamMap):
        ...
        if data_i[0] == 0:
            ...
            price = orderRc[3]
            vol_i = orderRc[4]
            if type_i== 1 and bsFlag == 1 and  OrderQue_test.bid.count > 0:
                bid_order[orderID] = [time_i,-1,price,OrderQue_test.bid.max_key(),0]
            if type_i== 1 and bsFlag == -1 and OrderQue_test.ask.count>0:
                ask_order[orderID] = [time_i,-1,price,OrderQue_test.ask.min_key(),0]
        if data_i[0] == 1:
            transRc = data_i[1:][:10]
            transID = transRc[-3]
            time_i = transRc[1]
            ...
            price = transRc[3]
            if type_i==1:
                if bidID in bid_order:
                    bid_order[bidID][1] = time_i
                    bid_order[bidID][-1] = 1
                if askID in ask_order:
                    ask_order[askID][1] = time_i
                    ask_order[askID][-1] = 1
                #如果同一笔订单被反复成交，该怎么处理？
        #OrderQue_test.handle(data_i)
    return ask_order,bid_order
ask_order,bid_order = trd_count()
#%%
df = pd.DataFrame.from_dict(bid_order, orient='index',columns=['start_time','end_time','price','bid','sign'])
df = df[df['start_time'] >= 93000000000000]
df = df[df['end_time'] < 145700000000000]
df['trdsign'] =  df['end_time'] > 0
df['timedelta'] = df['end_time'] - df['start_time']
df['pricedelta'] =  (df['price'] - df['bid']).abs()//100
df = df[df['pricedelta'].abs() <= 10]
df = df[df['timedelta'] < 5*1e09]
x = df.groupby(by = 'pricedelta').sign.mean()
#plt.plot(x.index,x.values)
#plt.scatter(x.index,x.values)
#plt.scatter(np.log(x.index+1),np.log(x.values))
from sklearn.linear_model import LinearRegression
model = LinearRegression()
model.fit(np.log(x.index+1).to_numpy().reshape(-1,1),np.log(x.values))
pre = np.exp(model.predict(np.log(x.index+1).to_numpy().reshape(-1,1)))
plt.scatter(x.index,x.values)
plt.plot(x.index,pre)
plt.show()