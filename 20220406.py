...
bid_order = {}
price_map = {}
count_all = np.zeros(10)
count_trd = np.zeros(10)
for data_i in getMkDataMult(dataStreamMap):
    ...
    if data_i[0]==0 :
        ...
        price = orderRc[3]
        vol_i = orderRc[4]
        if (time_i > 93000 * 1e9) and (time_i < 145700 * 1e9) and type_i==1:
            bs.append(bsFlag)
            if bsFlag == 1:
                bid_order[orderID] = price - OrderQue_test.bid.max_key()
                if np.abs(price - OrderQue_test.bid.max_key()) <= 900:
                    count_all[bid_order[orderID]//100]+=1
                #p.append(price)
                #p1.append(OrderQue_test.bid.max_key())
            else:
                ask_order[orderID] = price - OrderQue_test.ask.min_key()
                if np.abs(price - OrderQue_test.ask.min_key()) <= 900:
                    count_all[ask_order[orderID]//100] += 1
                #p.append(price)
                #p1.append(OrderQue_test.ask.min_key())
    if data_i[0] == 1:
        transRc = data_i[1:][:10]
        transID = transRc[-3]
        time_i = transRc[1]
        ...
        if (time_i > 93000 * 1e9) and (time_i < 145700 * 1e9) and type_i == 1:
            if bidID in bid_order :
                if np.abs(bid_order[bidID]) <= 900:
                    count_trd[bid_order[bidID] // 100] += (bid_order[bidID]!=-1)
                    bid_order[bidID] = -1

            if askID in ask_order:
                if np.abs(ask_order[askID]) <= 900:
                    count_trd[ask_order[askID] // 100] += (ask_order[askID]!=-1)
                    ask_order[askID] = -1
